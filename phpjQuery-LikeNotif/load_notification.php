...
if (isset($_POST['view'])) {
    ...
    $query = "SELECT user_content_like.user_id, content.description, user_details.user_name, user_details.user_image FROM user_content_like
        INNER JOIN content
    ...
    if ($_POST['view'] != '') {
        $select_query = "SELECT * FROM content WHERE user_id = :user_id";
        ...
        $result = $statement->fetchAll();
        foreach ($result as $row) {
            $update_query = "UPDATE user_content_like SET status = 'seen' WHERE content_id = :content_id AND status = :status";
